...
I18n.pluralize = function(count, scope, options) {
  ...
  if( options.wrap ) {
    options.count = "<" + options.wrap + ">" + count + "</" + options.wrap + ">";
    delete(options.wrap);
  } else {
...
I18n.relative = function(format, time) {
  var diff = Math.floor((Date.now() - time) / 1000);
  var type = ( diff < 0 )  ? "in" : "ago";
  diff = Math.abs(diff);

  if( diff < 60 ) {
    return I18n.p(diff, "js.seconds_" + type)
  } else if( diff < 3600 ) {
    return I18n.p(Math.floor(diff / 60), "js.minutes_" + type)
  } else if( diff < 86400 ) {
    return I18n.p(Math.floor(diff / 3600), "js.hours_" + type)
  } else if( diff < 604800 ) {
    return I18n.p(Math.floor(diff / 86400), "js.days_" + type)
  } else {
    return I18n.t("js.fallback_time", {time: I18n.l(format, time)});
  }
};
